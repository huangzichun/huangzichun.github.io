...
为了求解出这个$x^{*}$，我们可以直接求解$∇f(x^{*} )=0$，但是不容易直接求解出来的。然后我们可以采用迭代的方式，找出一组$x$的序列$x^{0},x^{1},...$满足当$k \rightarrow \infty$时，$f(x^{k}) \rightarrow f(x^{*})$。梯度下降便是这样的迭代算法，给定初始$x^{0}$，该算法的$x$序列设置为
